...
Input File Naming Conventions:
  ...
  BGEN:  chr1.bgen, chr2.bgen, ... (with chr1.sample, chr2.sample)

Output Files (example for bfile format):
...
Use in SAIGE-GENE (BGEN format):
  Rscript step2_SPAtests.R \
    ...
    --bgenFileIndex=gene_bfiles/chr1_genes.bgen.bgi \
    --sampleFile=gene_bfiles/chr1_genes.sample \
...
# Try to load plink2a module (adjust module name as needed for your system)
...
for chr in {1..22} X Y; do
    ...
    case "$DETECTED_FORMAT" in
        ...
        vcf)
            ...
            ;;
        bgen)
            # Try to get from .bgi index or log file
    ...
    # Build PLINK2a command based on input format
    ...
    PLINK_CMD="$PLINK_CMD --extract bed1 $REGION_FILE"
...
case "$OUTPUT_FORMAT" in
    bfile)
        ...
        echo ""
        echo "2. Convert to other formats if needed:"
        echo "   # To PGEN:"
        echo "   plink2a --bfile $OUTPUT_DIR/chr1_genes --make-pgen --out $OUTPUT_DIR/chr1_genes_pgen"
        echo ""
        echo "   # To VCF:"
        echo "   plink2a --bfile $OUTPUT_DIR/chr1_genes --export vcf bgz --out $OUTPUT_DIR/chr1_genes"
        echo ""
        echo "   # To BGEN:"
        echo "   plink2a --bfile $OUTPUT_DIR/chr1_genes --export bgen-1.2 --out $OUTPUT_DIR/chr1_genes"
        ;;
    ...
    pgen)
        ...
        echo "2. Convert to other formats if needed:"
        echo "   # To BFILE:"
        echo "   plink2a --pfile $OUTPUT_DIR/chr1_genes --make-bed --out $OUTPUT_DIR/chr1_genes_bfile"
        ;;
    ...
    vcf)
        ...
        echo ""
        echo "2. Query specific regions:"
        echo "   bcftools view -r 1:1000000-2000000 $OUTPUT_DIR/chr1_genes.vcf.gz"
        echo ""
        echo "3. Convert to other formats:"
        echo "   # To BFILE:"
        echo "   plink2a --vcf $OUTPUT_DIR/chr1_genes.vcf.gz --make-bed --out $OUTPUT_DIR/chr1_genes"
        ;;
    ...
    bgen)
        ...
        echo "     --LOCO=FALSE"
        echo ""
        echo "3. Convert to other formats if needed:"
        echo "   # To BFILE:"
        echo "   plink2a --bgen $OUTPUT_DIR/chr1_genes.bgen --sample $OUTPUT_DIR/chr1_genes.sample \\"
        echo "           --make-bed --out $OUTPUT_DIR/chr1_genes"
        ;;
esac
